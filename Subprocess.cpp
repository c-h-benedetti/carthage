...
void Subprocess::set_state(Subprocess::State s){
  ...
  for(const auto& func : this->subscribers){
    func(this->state);
  }
}
...
void Subprocess::subscribe_to(const std::function<void(State)>&& f){
  this->subscribers.push_back(std::move(f));
}
...
Subprocess::Subprocess(
  ...
  std::string log_path, 
  std::string p_name
) : command(std::move(cmd)), command_line_args(std::move(args)), log_file_path(std::move(log_path)), process_name(std::move(p_name)){
...
Subprocess::Subprocess(const Subprocess& s): 
  command(s.command), 
  _command_(s._command_), 
  command_line_args(s.command_line_args), 
...
Subprocess& Subprocess::operator=(Subprocess s){

  std::swap(this->command, s.command);
  std::swap(this->_command_, s._command_);
  std::swap(this->command_line_args, s.command_line_args);
  this->state = State::INITIALIZED;
  this->subscribers = {};
  std::swap(this->log_file_path, s.log_file_path);
  std::swap(this->process_name, s.process_name);
  this->pid = 0;
  memset(this->file_descriptors, 0, 2 * sizeof(int));
...
Subprocess::Subprocess(Subprocess&& s):
  command(std::move(s.command)),
  _command_(std::move(s._command_)),
  command_line_args(std::move(s.command_line_args)),
  state(State::INITIALIZED),
  subscribers({}),
  log_file_path(std::move(s.log_file_path)),
  process_name(std::move(s.process_name)),
  pid(0),
  file_descriptors{0},
  return_status(-1)
{}
...
Subprocess& Subprocess::operator=(Subprocess&& s){
  ...
  this->command = std::move(s.command);
  this->_command_ = std::move(s._command_);
  this->command_line_args = std::move(s.command_line_args);

  this->state = State::INITIALIZED;
  this->subscribers = {};

  this->log_file_path = std::move(s.log_file_path);
  this->process_name = std::move(s.process_name);
